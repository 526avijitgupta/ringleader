...
self.port.on("refreshed-lookups", function(data) {
  ...
  evt.initCustomEvent(
    data.signal, true, true, cloneInto(data.list, document.defaultView));
        ...
        for(idx in commands) {
          ...
          if (commands[idx].params && commands[idx].params.length > 0) {
            for (var index = 0; index < commands[idx].params.length; index++) {
              if (commands[idx].params[index].type.name === 'selection' &&
                  typeof commands[idx].params[index].type.data === 'string') {
                eval("commands[idx].params[index].type.data = " +
                     commands[idx].params[index].type.data);
              }
            }
          }
