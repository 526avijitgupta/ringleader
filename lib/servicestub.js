...
ServiceStub.prototype.modCommand = function(command) {
  ...
  try {
    ...
    if (command.execAction) {
      ...
      var result = {name: command.name};
      // var exec =
      var exec = function ServiceStub_exec(args, context) {
        alert('xyz');
      ...
      // result.exec = exec;
      command.exec = function(args, context) {
        let result = {name: this.name, args: this.args, src: 'ZAP'};
...
ServiceStub.prototype.hook = function () {
  readURI(this.url).then(function(data) {
    try {
      ...
      // Sending commands to main.js
      commands.forEach(function(command){
        if (command.hasOwnProperty('exec')) {
          command.exec += "";
        }
      });
      // console.log('ss-commands: ', commands);
