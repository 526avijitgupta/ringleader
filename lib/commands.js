...
//   var lookupData = [configManager.currentConfigs(), configManager.list()];
//   return lookupData;
// }

function setExec() {
  return function (args, context) {
    let result = {name: this.name, args: args, src: 'PnH'};
...
commands.forEach(function(command) {
  ...
  if (command.params && command.params.length != 0) {
    command.params[0].type.data = command.params[0].type.data();
  }
  commands_clone.push(obj);
});
...
exports.commands_clone = commands_clone;
